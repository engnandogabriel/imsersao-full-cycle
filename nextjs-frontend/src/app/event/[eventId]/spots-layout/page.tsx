...
export async function getSpots(eventId: string): Promise<{
  ...
  const { data: dataSpots } = await axios.get(
    `${process.env.NODE_API_URL}/events/${eventId}/spots`,
    {
      headers: {},
    }
  );
  const { data: dataEvent } = await axios.get(
    `${process.env.NODE_API_URL}/events/${eventId}`,
    {
      headers: {},
    }
  );
